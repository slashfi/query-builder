...
interface UpdateParams<S extends BaseDbDiscriminator> {
  ...
  fieldMappings:
    ...
    | undefined;
  values: Record<string, GenericAny>[] | undefined;
...
type UpdateResult<
  Params extends UpdateParams<S>,
  S extends BaseDbDiscriminator,
> = undefined extends Params['returning'] ? void : Record<string, GenericAny>[];
...
> = AllOf<
  [
    Params['values'] extends Record<string, GenericAny>[] ? true : false,
    Params['where'] extends ExpressionBase<DataTypeBoolean> ? true : false,
    Params['fieldMappings'] extends Record<
      string,
...
> {
  ...
  values: AnyOf<
    [
      Params['fieldMappings'] extends Record<string, GenericAny> ? true : false,
      IsAny<Params>,
    ]
  > extends true
    ? (
...
export function createUpdate<S extends BaseDbDiscriminator>(config: {
  ...
  function baseUpdate<Target extends GenericEntityTarget<S>>(
    ...
    const builder = {
      ...
      where: (fn) => {
        const targets: UpdateTargetList<typeof target.Table, GenericAny, S> = [
          { ...targetBase, isUsingAlias: true },
      ...
      then<TResult1 = UpdateResult<typeof params, S>, TResult2 = never>(
        onfulfilled?:
          ...
          | null,
        onrejected?:
          | ((reason: GenericAny) => TResult2 | PromiseLike<TResult2>)
          | undefined
          | null
        ...
        return config
          ...
          .then((res) => {
            ...
            return onfulfilled
              ? onfulfilled(transformedRes as GenericAny)
              : (transformedRes as TResult1);
          })
          .catch(onrejected);
      },
    } as UpdateBuilder<GenericAny, S>;
