...
import { expressionRowNumber } from './expressions/ExpressionRowNumber';
...
export interface AstNodeRepository<Node extends AstNode> {
  ...
  type: Node['type'];
  create: (...params: GenericAny[]) => Node;
  writeSql: (val: Node, parent: Node | undefined) => SqlString;
  isNode: (val: AstNode) => val is Node;
}

export function getAstNodeRepository(
  node: AstNode
): AstNodeRepository<GenericAny> {
  const nodes = [
