...
export async function runQueryResult<S extends BaseDbDiscriminator>(
  ...
  errorWithCorrectStack?: Error
) {
  const stack =
    (errorWithCorrectStack ?? new Error()).stack
      ?.split('\n')
      .slice(1)
      .map((line) => `-- ${line}`)
      .concat('')
  ...
  if (data?.throwsIfEmpty) {
    if (!res.length) {
      if (typeof data.throwsIfEmpty === 'boolean') {
        throw errorWithCorrectStack ?? new Error('Not found');
      }
...
>(
  ...
  : never {
  return transformRawResultToQuery(value._debug(), result) as GenericAny;
}
...
export function transformColumnForDataType(
  columnData: GenericAny,
  dataType: DataTypes
) {
  if (columnData === null && !isDataTypeNullable(dataType)) {
    throw new Error(
      `Item returned NULL on a non-nullable column for data type: ${JSON.stringify(dataType, undefined, 2)}`
    );
  }
  ...
  switch (finalDataType.type) {
    ...
    case 'decimal': {
      ...
      if (decimalType.mode === 'bigint') {
        if (typeof columnData === 'string') {
          try {
            return BigInt(columnData);
          } catch {
            throw new Error(
              `Failed to convert decimal to BigInt: ${columnData}`
            );
          }
        }
        if (typeof columnData === 'bigint') {
          return columnData;
        }
        throw new Error(
          `Data returned from SQL server did not match data type: decimal with bigint mode (got ${typeof columnData})`
        );
      }
      // Default string mode - no conversion needed
      if (typeof columnData !== 'string') {
        throw new Error(
          `Data returned from SQL server did not match data type: decimal with string mode (got ${typeof columnData})`
        );
      }
      return columnData;
    }
