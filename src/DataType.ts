...
import type { ConstantForDecimal } from './constants/ConstantForDecimal';
...
export function createDataTypeDecimal<
  Mode extends 'string' | 'bigint' = 'string',
...
>(options?: {
  ...
  ? DataTypeUnion<
      [DataTypeDecimal<Mode extends 'bigint' ? bigint : string>, DataTypeNull]
    >
  : DataTypeDecimal<Mode extends 'bigint' ? bigint : string> {
  const mode = options?.mode || 'string';
  const dataType = {
    class: 'data_type' as const,
    type: 'decimal' as const,
    mode: mode as 'string' | 'bigint' | undefined,
    baseExpression: undefined as GenericAny,
    constExpression: undefined as GenericAny,
    narrowedType: undefined as GenericAny,
  } as DataTypeDecimal<Mode extends 'bigint' ? bigint : string>;
...
>(
  ...
  return {
    ...
    columnDataTypes: finalColumnData as GenericAny,
    baseExpression: undefined as GenericAny,
    constExpression: undefined as GenericAny,
    narrowedType: undefined as GenericAny,
  };
...
export interface DataTypeDecimal<T extends string | bigint = string>
  extends DataTypeBase {
  type: 'decimal';
  mode?: T extends bigint ? 'bigint' : 'string';
  narrowedType: NonNullable<T>;
  baseExpression: ExpressionBase<DataTypeDecimal<NonNullable<T>>>;
  constExpression: ConstantForDecimal<NonNullable<T>>;
}
...
export interface DataTypeJson<
  T extends GenericAny = { [Key in string]: GenericAny },
> extends DataTypeBase {
  type: 'json';
  narrowedType: NonNullable<T>;
...
/**
 * Union of all primitive data type names (the `type` property of DataTypes).
 * Excludes compound types like 'union', 'tuple', 'tabular_columns'.
 */
export type PrimitiveDataTypeName = Exclude<
  DataTypes['type'],
  'union' | 'tuple' | 'tabular_columns'
>;
...
export const allDataTypes = [
  ...
  createDataTypeDecimal({ isNullable: true, mode: 'bigint' as const }),
  ...
  createDataTypeDecimal({ isNullable: false, mode: 'bigint' as const }),
  createDataTypeArray(createDataTypeBoolean({ isNullable: false }), {
    isNullable: false,
  }),
  createDataTypeUnion([]),
...
>(options: {
  ...
  return filterUndefined(options.pick
